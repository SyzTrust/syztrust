...
type StateValues [sha1.Size]byte

func BytesToStateValues(pieces []byte) StateValues {
	var stateValues StateValues
	copy(stateValues[:], pieces)
	return stateValues
}

func HashState(pieces ...[]byte) StateValues {
	h := sha1.New()
	for _, data := range pieces {
		h.Write(data)
	}
	var stateValues StateValues
	copy(stateValues[:], h.Sum(nil))
	return stateValues
}
