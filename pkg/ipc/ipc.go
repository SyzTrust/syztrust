...
func GetSimpleEnv(config *Config, pid int, nextFileNum int) (*Env, error) {
	env := &Env{
		pid:         pid,
		NextFileNum: nextFileNum,
		out:         make([]byte, outputSize), // save callReply structure
		FailureCnt:  0,
		config:      config,
	}
	return env, nil
...
func (env *Env) ExecPayload(payload []byte, p *prog.Prog) (output []byte, info *ProgInfo, hanged bool, err0 error) {

	_, err := env.Conn.Write(payload)
	if err != nil {
	...
	reader := bufio.NewReader(env.Conn)

	arr := make([]byte, 4)
	for i := 0; i < 4; i++ {
		arr[i], _ = reader.ReadByte()
	}
	size, _ := readUint32(&arr)
	if err != nil {
		log.Logf(0, err.Error())
		return nil, nil, false, err
	}
	if size == 0xffffffff {
		log.Fatalf("Python socket closed.")
	}
	...
	for i := uint32(0); i < size; i++ {
		env.out[i], _ = reader.ReadByte()
	}
	info, err = env.parseOutput(p)
	if err != nil {
		log.Fatal(err)
		return nil, nil, false, err
	}
...
func (env *Env) parseOutput(p *prog.Prog) (*ProgInfo, error) {
	...
	dictCnt, _ := readUint32(&out)
	if dictCnt != 0 {
		info.States = make([]map[uint32][][]byte, dictCnt)
		for i := uint32(0); i < dictCnt; i++ {
			typeNums, _ := readUint32(&out)
			stateMap := make(map[uint32][][]byte, 1)
			for j := uint32(0); j < typeNums; j++ {
				typeIndex, _ := readUint32(&out)
				handleNums, _ := readUint32(&out)
				
				handleArr := make([][]byte, handleNums)
				for k := uint32(0); k < handleNums; k++ {
					dataSize, _ := readUint32(&out)

					handleArr[k] = out[:dataSize]
					out = out[dataSize:]
				}
				stateMap[typeIndex] = handleArr
			}
			info.States[i] = stateMap
		}
	}
	...
	for i := uint32(0); i < ncmd; i++ {
		...
		if reply.index != extraReplyIndex {
			if int(reply.index) >= len(info.Calls) {
				log.Logf(0, "bad call %v index %v/%v", i, reply.index, len(info.Calls))
				return nil, nil
		...
		inf.Comps = comps
		inf.State = make(map[uint32][][]byte)
		if len(info.States) > int(i) {
			if _, ok := info.States[i][1]; ok {
				inf.State[1] = info.States[i][1]
			}
			if _, ok := info.States[i][2]; ok {
				inf.State[2] = info.States[i][2]
			}
		}
	}
