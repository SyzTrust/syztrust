...
func (p *Prog) GeneratePayload() ([]byte, []byte) {
	p.debugValidate()
	ctx := &serializer{
		target:     p.Target,
		buf:        new(bytes.Buffer),
		header:     new(bytes.Buffer),
		argDataBuf: new(bytes.Buffer),
		vars:       make(map[*ResultArg]int),
		verbose:    true,
		varSeq:     1, // bias 1 : skip 0 which means "tmp index" in payload
		headerLen:  0,
	}
	ctx.argDataBuf.Write([]byte{0xff, 0xff, 0xff, 0xff})
	// calculate header length
	for _, c := range p.Calls {
		ctx.headerLen += 8               // syscall_id and argType
		ctx.headerLen += 4 * len(c.Args) // argInfo
	}
	//print(ctx.headerLen)
	for _, c := range p.Calls {
		// clear cache
		ctx.argTypes = 0
		ctx.argIndex = 0
		ctx.argInfoBuf = new(bytes.Buffer)
		ctx.call(c)
	}

	return ctx.buf.Bytes(), append(ctx.header.Bytes(), ctx.argDataBuf.Bytes()...)
}
...
type serializer struct {
	target     *Target
	buf        *bytes.Buffer // sys calls : human-readable (prog)
	header     *bytes.Buffer // all syscalls' ID, argType, argInfo
	headerLen  int           // the length of header
	argInfoBuf *bytes.Buffer // one syscall's argInfo
	curArgType int
	argTypes   int
	argIndex   int
	argDataBuf *bytes.Buffer
...
func (ctx *serializer) addArgType(t int) {
	//print("Add ",t," to arg_type ")
	ctx.curArgType = t
	ctx.argTypes |= t << (ctx.argIndex * 4)
	ctx.argIndex++
}

// addArgType9ToHeader: Used for adding 4 bytes information in payload-arg-info section
func (ctx *serializer) addArgType9ToHeader(size int) {
	ctx.addArgType(ArgSaveStruct)
	offset := ctx.headerLen + len(ctx.argDataBuf.Bytes())
	arg := (offset << 16) | (size & 0xffff)
	ctx.bufferAddInt(ctx.argInfoBuf, arg)
}

// addSubArgType: Used for adding 4 bytes information in payload-data section
func (ctx *serializer) addSubArgType(inType, outType, argType, argSize int) {
	subArgType := inType | (outType << 4) | (argType << 8) | (argSize << 16)
	ctx.bufferAddInt(ctx.argDataBuf, subArgType)
}

func (ctx *serializer) bufferAddInt(buf *bytes.Buffer, num int) {
	x := int32(num)
...
func (ctx *serializer) call(c *Call) {
	varID := 0 // index 0 is "not care" in payload
	if c.Ret != nil && len(c.Ret.uses) != 0 {
		varID = ctx.allocVarID(c.Ret)
		ctx.printf("r%v = ", varID)
	}
	ctx.printf("%v(", c.Meta.Name)

	// fill syscall id with specified index to store possible return value
	ctx.bufferAddInt(ctx.header, (varID<<16)|int(c.Meta.NR))
	...
	for i, a := range c.Args {
		...
		// only the outer args have effects on the syscall argsType
		// top args are placed in header while non-top args are placed in argDataBuf section.
		// Note that the data in argDataBuf should be represented using extra 4 or 8 or 12 bytes if arg type is 9
		ctx.argLevel = -1
		ctx.arg(a)
	}
	ctx.printf(")")

	// load syscall arg types and arg info
	ctx.bufferAddInt(ctx.header, ctx.argTypes)
	ctx.header.Write(ctx.argInfoBuf.Bytes())
...
func (ctx *serializer) arg(arg Arg) {
	...
	if arg == nil {
		ctx.isPointee = false
		ctx.printf("nil")
		if ctx.argLevel == 0 { //[GEN] top level
			ctx.addArgType(ArgNull)
			ctx.bufferAddInt(ctx.argInfoBuf, 0)
		} else {
			ctx.addSubArgType(PureSub, PureSub, ConstSub, 4)
			ctx.bufferAddInt(ctx.argDataBuf, 0)
		}
		ctx.argLevel--
		return
	}
...
func (a *ConstArg) serialize(ctx *serializer) {
	if ctx.argLevel == 0 { // top -> -1 ; const -> 0
		ctx.addArgType(Arg32Bit)
		ctx.bufferAddInt(ctx.argInfoBuf, int(a.Val))
	} else if ctx.argLevel == 1 { // top -> -1 ; pointer -> 0 ; const -> 1
		ctx.addArgType(ArgTempMalloc)
		offset := ctx.headerLen + len(ctx.argDataBuf.Bytes())
		ctx.bufferAddInt(ctx.argInfoBuf, (offset&0xffff)|(4<<16))
		ctx.bufferAddInt(ctx.argDataBuf, int(a.Val))
	} else { //level >= 2 means that this const is in structure or union
		if ctx.isPointee {
			// If it is a pointee, then regard it as buf[4]
			ctx.addSubArgType(PureSub, PureSub, BufSub, 4)
		} else {
			ctx.addSubArgType(PureSub, PureSub, ConstSub, 4)
		}
		ctx.bufferAddInt(ctx.argDataBuf, int(a.Val))
...
func (a *PointerArg) serialize(ctx *serializer) {
	//print("PointerArg ", a.Type().Name(), " at level ", ctx.argLevel, " size = ", a.Res.Size(), "\n")
	if a.IsSpecial() {
		// special pointer can be treated as int32
		if ctx.argLevel == 0 {
			ctx.addArgType(Arg32Bit)
			ctx.bufferAddInt(ctx.argInfoBuf, int(a.Address))
		} else {
			ctx.addSubArgType(PureSub, PureSub, ConstSub, 4)
			ctx.bufferAddInt(ctx.argDataBuf, int(a.Address))
		}
	...
	//[GEN] should not handle the pointer itself at this moment
	ctx.printf("=")
	if target.isAnyPtr(a.Type()) {
		ctx.printf("ANY=")
	}
	if ctx.argLevel == 1 { // top : -1 -> pointer : 0 -> pointer : 1
		ctx.addArgType9ToHeader(int(a.Size()))
	}
	ctx.isPointee = true
	ctx.arg(a.Res)
	ctx.isPointee = false
...
func (a *DataArg) serialize(ctx *serializer) {
	...
	if a.Dir() == DirOut {
		ctx.printf("\"\"/%v", a.Size())
		// e.g. destData buffer[out]
		if ctx.argLevel == 1 { // top -> -1 ; pointer -> 0 ; data -> 1
			ctx.addArgType(ArgSaveMalloc)
			ctx.bufferAddInt(ctx.argInfoBuf, int(a.Size())) // low 16bits for size, high 16bits for dst
		} else {
			ctx.addSubArgType(PureSub, PureSub, BufSub, int(a.Size()))
			ctx.bufferAddInt(ctx.argDataBuf, 0)
	...
	if ctx.argLevel == 1 { // top level: none-> -1 ; pointer -> 0 ; data -> 1
		offset := ctx.headerLen + len(ctx.argDataBuf.Bytes())
		ctx.addArgType(ArgTempMalloc)
		ctx.bufferAddInt(ctx.argInfoBuf, offset|(sz<<16))
	} else { // buffer data in structure or union
		ctx.addSubArgType(PureSub, PureSub, BufSub, sz)
	}
	ctx.argDataBuf.Write(data)
...
func (a *GroupArg) serialize(ctx *serializer) {
	...
	if ctx.argLevel == 1 { // top level -> -1 ; pointer -> 0 ; group -> 1
		ctx.addArgType9ToHeader(int(a.Size())) // TODO: take in/out into consideration
	} else {
		if ctx.isPointee {
			ctx.addSubArgType(PureSub, PureSub, StructPtrSub, int(a.Size()))
		} else {
			ctx.addSubArgType(PureSub, PureSub, StructSub, int(a.Size()))
		}
	}
...
func (a *ResultArg) serialize(ctx *serializer) {
	//print("ResultArg ", a.Type().Name(), " at level ", ctx.argLevel, ", \n")
	varID := -1
	if len(a.uses) != 0 {
		varID = ctx.allocVarID(a)
		ctx.printf("<r%v=>", varID)
		//print("<r",varID,"=> ")
	}
	if a.Res == nil {
		ctx.printf("0x%x", a.Val)
		//print(a.Val,"(nil) ")
		// [GEN] this means const value
		if ctx.argLevel == 1 { // top : -1 -> pointer : 0 -> res : 1
			if varID == -1 { // no out
				ctx.addArgType9ToHeader(4)
				ctx.bufferAddInt(ctx.argDataBuf, int(a.Val))
			} else {
				ctx.addArgType9ToHeader(4)
				ctx.bufferAddInt(ctx.argDataBuf, int(a.Val))
				ctx.bufferAddInt(ctx.argDataBuf, varID)
			}
		} else {
			if varID == -1 {
				ctx.addSubArgType(ConstSub, PureSub, ResSub, 4)
				ctx.bufferAddInt(ctx.argDataBuf, int(a.Val))
			} else {
				ctx.addSubArgType(ConstSub, IndexSub, ResSub, 4)
				ctx.bufferAddInt(ctx.argDataBuf, int(a.Val))
				ctx.bufferAddInt(ctx.argDataBuf, varID)
			}
		}
	...
	ctx.printf("r%v", id)
	//print("id=",id, " ")
	//[GEN] now we get in(id) and out(varID)
	if ctx.argLevel == 0 { //top : -1 -> directly result value rX
		if varID == -1 {
			varID = 0
		}
		ctx.addArgType(ArgUseGlobal)
		ctx.bufferAddInt(ctx.argInfoBuf, (id&0xffff)|((varID&0xffff)<<16))
	} else if ctx.argLevel == 1 { // top : -1 -> pointer : 0 -> res : 1
		
	} else { // in structure or union
		if varID == -1 {
			ctx.addSubArgType(IndexSub, PureSub, ResSub, 4)
			ctx.bufferAddInt(ctx.argDataBuf, id)
		} else {
			ctx.addSubArgType(IndexSub, IndexSub, ResSub, 4)
			ctx.bufferAddInt(ctx.argDataBuf, id)
			ctx.bufferAddInt(ctx.argDataBuf, varID)
		}
	}

	// what do opdiv and opadd mean?
	if a.OpDiv != 0 {
