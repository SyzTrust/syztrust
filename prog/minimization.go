...
func Minimize(p0 *Prog, callIndex0 int, crash bool, pred0 func(*Prog, int) bool) (*Prog, int) {
	log.Logf(0, "in Minimize function")
...
// whether it is equal to the original program or not. If it is equivalent then
// the simplification attempt is committed and the process continues.
func MinimizeNoArg(p0 *Prog, callIndex0 int, crash bool, pred0 func(*Prog, int) bool) (*Prog, int) {
	log.Logf(0, "in Minimize function")
	pred := func(p *Prog, callIndex int) bool {
		p.debugValidate()
		return pred0(p, callIndex)
	...
	// Try to remove all calls except the last one one-by-one.
	log.Logf(0, "start remove calls")
	p0, callIndex0 = removeCallsNew(p0, callIndex0, crash, pred)
...
func removeCallsNew(p0 *Prog, callIndex0 int, crash bool, pred func(*Prog, int) bool) (*Prog, int) {
	targetP := p0.Clone()
	...
	for i := len(p0.Calls) - 1; i >= 0; i-- { 
		...
		//logRemovedCall(targetP, i, 0)
		removedCalls = append(removedCalls, i)
		p0 = p
		callIndex0 = callIndex
	}
	// logRemovedCall(targetP, removedCalls, 1)
	return p0, callIndex0
}

func logRemovedCall(p *Prog, removedCalls []int, removeFlag int) {
	file, err := os.OpenFile("${project_root_dir}/workdir/removeCalls.log", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666) // NOTE: please manually update ${project_root_dir}
	if err != nil {
	...
	if removeFlag == 0 {
		...
		for i := 0; i < len(p.Calls); i++ {
			write.WriteString(p.Calls[i].Meta.CallName + " ")
		}
		write.WriteString("\n")
		write.WriteString("removed calls: " + strconv.Itoa(len(removedCalls)) + "\n")
		if len(removedCalls) > 0 {
			for j := 0; j < len(removedCalls); j++ {
				write.WriteString(strconv.Itoa(removedCalls[j]) + " ")
			}
