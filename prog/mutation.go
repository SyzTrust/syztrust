...
func (p *Prog) MutateState(rs rand.Source, ncalls int, ct *ChoiceTable, corpus []*Prog, targetCall int) {
	...
	// the main mutation strategies
	for stop, ok := false, false; !stop; stop = ok && len(p.Calls) != 0 && r.oneOf(3) {
		switch {
		case r.oneOf(5):
			// Not all calls have anything squashable,
			// so this has lower priority in reality.
			ok = ctx.squashAnyState(targetCall)
		case r.nOutOf(1, 100):
			ok = ctx.spliceState(targetCall)
		case r.nOutOf(20, 31):
			ok = ctx.insertCallState(targetCall)
		case r.nOutOf(10, 11):
			ok = ctx.mutateArgState(targetCall)
		default:
			ok = ctx.removeCallState(targetCall)
		}
	}
	p.sanitizeFix()
	p.debugValidate()
...
func (ctx *mutator) spliceState(targetCall int) bool {
	p, r := ctx.p, ctx.r
	...
	p0c := p0.Clone()
	idx := r.Intn(len(p.Calls)-targetCall) + targetCall
...
func (ctx *mutator) squashAnyState(targetCall int) bool {
	p, r := ctx.p, ctx.r
	complexPtrs := p.complexPtrsState(targetCall)
...
func (ctx *mutator) insertCallState(targetCall int) bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) >= ctx.ncalls {
		return false
	}
	idx := r.biasedRand(len(p.Calls)+1-targetCall, 5) + targetCall
	var c *Call
	if idx < len(p.Calls) {
		c = p.Calls[idx]
	}
...
func (ctx *mutator) removeCallState(targetCall int) bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) == 0 {
		return false
	}
...
func (ctx *mutator) mutateArgState(targetCall int) bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) == 0 {
		return false
	}
	idx := chooseCallState(p, r, targetCall)
	if idx < 0 {
		return false
	}
	c := p.Calls[idx]
...
func chooseCallState(p *Prog, r *randGen, targetCall int) int {
	var prioSum float64
