...
type Fuzzer struct {
	...
	machineInfo   []byte
	newMaxStates state.MapStateSlice
}
...
func (serv *RPCServer) Connect(a *rpctype.ConnectArgs, r *rpctype.ConnectRes) error {
	...
	r.ProgsDir = serv.cfg.ProgsDir
	r.PayloadDir = serv.cfg.PayloadDir
	r.SigCovStateDir = serv.cfg.SigCovStateDir
	r.BoardServer = serv.cfg.BoardServer
	r.CommandPython = serv.cfg.CommandPython
	r.CorpusFile = serv.cfg.CorpusFile
	r.TraceID = serv.cfg.TraceID
	r.StateFlag = serv.cfg.StateFlag
	r.HandleDivision = serv.cfg.HandleDivision
	if serv.mgr.rotateCorpus() && serv.rnd.Intn(5) == 0 {
		...
		f.newMaxStates = serv.maxStates.Copy()
	}
	return nil
}
...
func (serv *RPCServer) Poll(a *rpctype.PollArgs, r *rpctype.PollRes) error {
	...
	newMaxStates := serv.maxStates.Diff(a.MaxStates.Deserialize())
	if !newMaxStates.Empty() {
		serv.maxStates.Merge(newMaxStates)
		for _, f1 := range serv.fuzzers {
			if f1 == f || f1.rotated {
				continue
			}
			f1.newMaxStates.Merge(newMaxStates)
		}
	}
	...
	log.Logf(4, "poll from %v: candidates=%v inputs=%v maxsignal=%v maxStates = %v",
		a.Name, len(r.Candidates), len(r.NewInputs), len(r.MaxSignal.Elems), len(r.MaxStates.Statevs))
	return nil
}
